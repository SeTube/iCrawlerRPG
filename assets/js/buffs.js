var Buffs = function() {
	...
	self.updateTemporaryBuffs = function(decrease) {
		...
		if (aegisTimeLeft !== 0) {
			...
			document.getElementById("temporary").innerHTML += '<li class="list-group-item list-group-item-info"><span class="badge">' + Math.round(aegisTimeLeft) + '</span>庇佑</li>';
		}

		if (barrierLeft !== 0) {
			document.getElementById("temporary").innerHTML += '<li class="list-group-item list-group-item-info"><span class="badge">' + Math.round(barrierLeft) + '</span>屏障</li>';
		}
		...
		if (rageTimeLeft !== 0) {
			...
			document.getElementById("temporary").innerHTML += '<li class="list-group-item list-group-item-info"><span class="badge">' + Math.round(rageTimeLeft) + '</span>愤怒</li>';
		}
	};
	...
	self.updateToggleableBuffs = function() {
		...
		if (castFireballInBattle || upgrades.isUpgradePurchased("autoshoot")) {
			...
			document.getElementById("toggleable").innerHTML += '<button type="button" class="list-group-item" onClick="buffs.toggleBuff(\'castFireballInBattle\')"><span class="badge">' + toggleStatusText + '</span>自动射击</button>';
		}
		...
		if (castCureInBattle || upgrades.isUpgradePurchased("battlehealing")) {
			...
			document.getElementById("toggleable").innerHTML += '<button type="button" class="list-group-item" onClick="buffs.toggleBuff(\'castCureInBattle\')"><span class="badge">' + toggleStatusText + '</span>战斗治疗</button>';
		}
		...
		if (autoBarrierCast || upgrades.isUpgradePurchased("barriercast")) {
			...
			document.getElementById("toggleable").innerHTML += '<button type="button" class="list-group-item" onClick="buffs.toggleBuff(\'autoBarrierCast\')"><span class="badge">' + toggleStatusText + '</span>屏障释放</button>';
		}
	};
	...
	self.updatePermanentBuffs = function() {
		...
		if (deathPenaltyReduction !== 0) {
			document.getElementById("permanent").innerHTML += '<li class="list-group-item"><span class="badge">' + deathPenaltyReduction + '%</span>死亡惩罚降低</li>';
		}
		if (exceliaMultiplier !== 1) {
			document.getElementById("permanent").innerHTML += '<li class="list-group-item"><span class="badge">x' + exceliaMultiplier + '</span>技能点获取</li>';
		}
		if (exceliaSavedOnDeath !== 0) {
			document.getElementById("permanent").innerHTML += '<li class="list-group-item"><span class="badge">' + exceliaSavedOnDeath + '%</span>死亡后保存技能点</li>';
		}
		if (manaPerSecond !== 0) {
			document.getElementById("permanent").innerHTML += '<li class="list-group-item"><span class="badge">+' + manaPerSecond + '</span>探索时每秒法力恢复</li>';
		}
		if (explorationSpeedMultiplier !== 1) {
			document.getElementById("permanent").innerHTML += '<li class="list-group-item"><span class="badge">x' + explorationSpeedMultiplier + '</span>探索速度</li>';
		}
		if (restingMultiplier !== 1) {
			document.getElementById("permanent").innerHTML += '<li class="list-group-item"><span class="badge">x' + restingMultiplier + '</span>休息速度</li>';
		}
		if (spellLevelingMultiplier !== 1) {
			document.getElementById("permanent").innerHTML += '<li class="list-group-item"><span class="badge">x' + spellLevelingMultiplier + '</span>法术等级提升</li>';
		}
		if (levelingSpeedMultiplier !== 1) {
			document.getElementById("permanent").innerHTML += '<li class="list-group-item"><span class="badge">x' + levelingSpeedMultiplier + '</span>属性经验获取</li>';
		}
	};
