var System = function() {
    ...
    self.importGame = function() {
        ...
        try {
            ...
            if (confirm("你确定要导入这个存档吗? 你之前的存档将会被清除.")) {
                localStorage.clear();
    ...
    var loadIdleButton = function() {
        if (idleMode) {
            document.getElementById("idleSwitch").innerHTML = '<button class="btn btn-success" onClick="system.toggleIdle()">挂机 开</button>';
        }
        else {
            document.getElementById("idleSwitch").innerHTML = '<button class="btn btn-danger" onClick="system.toggleIdle()">挂机 关</button>';
        }
    };
};
